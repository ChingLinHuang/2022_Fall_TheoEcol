...
# model
R_star_model <- function(times, state, parms) {
...
state <- c(N1 = 1, N2 = 1, R = 0.05)  
parms <- c(a1 = 0.4, a2 = 0.6, e1 = 1, e2 = 1, d = 0.01, S0 = 0.1)

pop_size <- ode(func = R_star_model, times = times, y = state, parms = parms)
plot(N2 ~ N1, data = pop_size, type = "l")
tail(pop_size)
